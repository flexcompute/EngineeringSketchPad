...
// Cross product axb = c
void cross_DoubleVal(const double a[], const double b[], double c[]);
...
// Add all a mapAttrToIndex structure keywords to an error message
void print_mapAttrError(void *aimInfo, const mapAttrToIndexStruct *attrMap);
...
// Retrieve the string following a capsSpline tag
int retrieve_CAPSSplineAttr(ego geomEntity, const char **string);
...
// Create a mapping between unique capsSpline attribute names and an index value
int create_CAPSSplineAttrToIndexMap(int numBody, ego bodies[], int attrLevel,
                                    mapAttrToIndexStruct *attrMap);
