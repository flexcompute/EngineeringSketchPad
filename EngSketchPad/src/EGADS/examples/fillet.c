...
int main(int argc, char *argv[])
{
  ...
  double size, box[6];
  ego    context, model, newModel, geom, body, *bodies, *edges, *faces, *fedge;
  ...
  if (argc == 3) {
    /* fillet all of them */
    printf(" \n");
    printf(" EG_fillet         = %d\n", EG_filletBody(bodies[0], nedge, edges, 
                                                      size, &body, &senses));
    printf(" EG_getBodyTopos   = %d\n", EG_getBodyTopos(body, NULL, FACE,
                                                        &n, &faces));
    for (i = 0; i < n; i++)
      printf("  faceMap %d: %d %d\n", i+1, senses[2*i  ], senses[2*i+1]);
    ...
    printf("\n\n");
    /* fillet each of them */
    printf(" EG_fillet         = %d\n", EG_filletBody(bodies[0], argc-3, fedge,
                                                      size, &body, NULL));
    free(fedge);
  }
